...
def main():
    ...
    filtered_wind_dat = wind_dat[
        (wind_dat["timestamp"] >= start) & (wind_dat["timestamp"] <= end)
    ]
    ...
    ch4_measurements = pd.read_csv(dir + "ch4_exp.csv")
    ch4_measurements["timestamp"] = pd.to_datetime(
        ch4_measurements["timestamp"], utc=True
    )
    ch4_filtered = ch4_measurements[
        (ch4_measurements["timestamp"] >= start)
        & (ch4_measurements["timestamp"] <= end)
    ]
    ...
    for i in range(n_sources):
        ...
        # run the simulation
        ch4 = sp.simulate()
        plot(sp, sensor_names, ch4_filtered, plotting_dir, source_names[i])
...
def plot(sp, sensor_names, ch4_measurements, plotting_dir, source_name):
    t, n_sensors = np.shape(sp.ch4_obs)

    fig, ax = plt.subplots(2, 4, figsize=(10, 10))
    m = sp.ch4_obs.max()
    obs_max  = ch4_measurements[sensor_names].max().max()
    m = max(m, obs_max)
    ...
    for i in range(0, n_sensors):
        ...
        ax[row][col].plot(
            times,
            ch4_measurements[sensor_names[i]].values,
            label="Observed",
            color="red",
        )
